...
def export_calendar_ics(plan: dict, filepath: Path, timezone: str = "America/New_York"):
    ...
    # ─── Weekly meal-planner routine reminders ────────────────────────────
    # Adds two recurring events with notifications so the user is nudged to:
    #   1. Review the auto-generated plan
    #   2. Send the grocery list to their Kroger pickup cart
    # These show up on Skylight, Google Calendar, Apple Calendar, etc.
    plan_url = "https://tosterl.github.io/Meal-Planner-Grociery-List/"
    sunday = start_date - timedelta(days=1)  # the Sunday before the plan starts
    sunday_str = sunday.strftime("%Y%m%d")

    # 1) Review the meal plan — Sunday 9:00 AM
    lines.extend([
        "BEGIN:VEVENT",
        f"UID:mealplan-review-{sunday_str}@mealplanner",
        f"DTSTAMP:{now_stamp}",
        f"DTSTART;TZID={timezone}:{sunday_str}T090000",
        f"DTEND;TZID={timezone}:{sunday_str}T091500",
        "SUMMARY:📋 Review this week's meal plan",
        f"DESCRIPTION:A fresh meal plan was just generated. Open the meal planner to review, swap any recipes, and approve before shopping.\\n\\nApp: {plan_url}",
        f"URL:{plan_url}",
        "STATUS:CONFIRMED",
        "BEGIN:VALARM",
        "ACTION:DISPLAY",
        "DESCRIPTION:Time to review your meal plan!",
        "TRIGGER:-PT5M",
        "END:VALARM",
        "END:VEVENT",
    ])

    # 2) Send grocery list to Kroger — Sunday 11:00 AM
    lines.extend([
        "BEGIN:VEVENT",
        f"UID:mealplan-kroger-{sunday_str}@mealplanner",
        f"DTSTAMP:{now_stamp}",
        f"DTSTART;TZID={timezone}:{sunday_str}T110000",
        f"DTEND;TZID={timezone}:{sunday_str}T111500",
        "SUMMARY:🛒 Send grocery list to Kroger pickup",
        f"DESCRIPTION:Open the meal planner → Grocery tab → click 'Send to Kroger'. Review the matched products, then submit for pickup.\\n\\nApp: {plan_url}",
        f"URL:{plan_url}",
        "STATUS:CONFIRMED",
        "BEGIN:VALARM",
        "ACTION:DISPLAY",
        "DESCRIPTION:Time to send your grocery list to Kroger!",
        "TRIGGER:-PT10M",
        "END:VALARM",
        "END:VEVENT",
    ])
